...
export default function SendMessage({ sellerId = null }) {
  ...
  const fetchMessages = async (chatId) => {
    ...
    try {
      const res = await axios.get(`${API_CHAT}/${messageId}`, {
        headers: { Authorization: `Bearer ${token}` },
      });
      setMessages(res.data.messages || []);

      // mark messages as seen
      socket?.emit("messageSeen", { messageId, userId: user._id });

      // update unread locally
      setChatList((prev) => prev.map((c) => (c._id === messageId ? { ...c, unreadCount: 0 } : c)));
    } catch (err) {
